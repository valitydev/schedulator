...
import dev.vality.schedulator.ScheduleTestData;
import dev.vality.schedulator.cron.DateAdjuster;
import dev.vality.schedulator.cron.ExcludeHolidayAdjuster;
...
import static dev.vality.damsel.base.DayOfWeek.*;
import static dev.vality.damsel.base.Month.*;
...
public class SchedulerUtilTest {
    ...
    public void testToQuartzFormat() {
        ...
        List<String> cronList = SchedulerUtil.buildCron(schedule,
                Optional.ofNullable(dev.vality.damsel.base.DayOfWeek.Tue));
        assertEquals(1, cronList.size());
        assertEquals("* * * ? * 1,5,7 *", cronList.get(0));

        cronList = SchedulerUtil.buildCron(schedule,
                Optional.ofNullable(dev.vality.damsel.base.DayOfWeek.Mon));
        assertEquals(1, cronList.size());
